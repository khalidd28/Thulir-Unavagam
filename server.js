...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const finalCategory = category || meal_type;
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          message: "Required menu details are missing."
        });
      }
...
app.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({
        success: false,
        message: "Unable to load admin menu."
      });
    }
  }
);
...
app.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // A food item that already belongs to an order
      // cannot be deleted because of the foreign key.
      if (error.code === "ER_ROW_IS_REFERENCED_2") {

        return res.status(400).json({
          success: false,
          message:
            "This menu item is already used in an order and cannot be deleted."
        });
      }
